...
class SUDOKU():
    ...
    def __init__(self, debug_flg, json_flg):
        ...
        #self.ocr_build = pyocr.builders.TextBuilder(tesseract_layout=10)
    ...
    def serch_rects(self):
        ...
        for cnt, hrchy in zip(contours, hierarchy[0]):
            #print(cv2.contourArea(cnt), SUDOKU.__cut_area_rate* self.origin_fig_area)
            rect_area = cv2.contourArea(cnt)
            if (rect_area > SUDOKU.__cut_area_rate* self.origin_fig_area)\
                 and (hrchy[3] != -1): # 面積が小さいものは除く # ルートノードは除く
                rect = cv2.minAreaRect(cnt)
                rect_points = cv2.boxPoints(rect).astype(int)
                rects.append(rect_points)

                #print(rect_area, target_rect_area)
                if target_rect_area < rect_area:
                    target_rect_area = rect_area
                    target_rect = cv2.boxPoints(rect).astype(int)
        
        margin_rate   =  SUDOKU.__sudoku_area_margin
        margin_width  = abs(target_rect[0][0] - target_rect[2][0]) * margin_rate
        margin_height = abs(target_rect[0][1] - target_rect[2][1]) * margin_rate

        rects = [rect for rect in rects if (rect[0][0]  > target_rect[0][0] - margin_width)  and\
                                           (rect[0][1]  > target_rect[0][1] - margin_height) and\
                                           (rect[2][0]  < target_rect[2][0] + margin_width)  and\
                                           (rect[2][1]  < target_rect[2][1] + margin_height)]
    ...
    def read_cell_number(self, id):
        ...
        txt = self.ocr_tool.image_to_string(img, lang="eng",\
                                        builder=self.ocr_build)
